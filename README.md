...
```mermaid
flowchart TB
  API[문학 서비스 FastAPI\nprocess 엔드포인트] --> REQ[요청 JSON 파싱\nsubject topK base64]
  REQ --> DECODE[base64 디코딩]
  DECODE --> TMP[임시 파일 저장\nquestion png passage png]

  subgraph INIT[서비스 시작 시 로드]
    EMB[OpenAIEmbeddings\ntext embedding 3 small]
    IDX[(FAISS 인덱스 로드)]
    EMB --> IDX
    IDX --> RET[Retriever MMR\nk 5 threshold 05]

    TAGJSON[(literature tagged json)]
    TAGJSON --> TAGDICT[태그 딕셔너리 생성]

    IMGROOT[(기출 이미지 폴더)]
  end

  subgraph PIPE[문학 파이프라인 pipeline literature]
    direction TB
    ...
    %% 2 지문 OCR 분할 정교화
    PIMG --> SPLIT[지문 이미지 분할\n4 parts]
    SPLIT --> PARTOCR[분할 OCR 반복\nGPT4o Vision]
    PARTOCR --> MERGE[OCR 결과 병합]
    MERGE --> REFINE[정교화\n기호 밑줄 복원]
    REFINE --> VERIFY[기호 검증 수정]
    VERIFY --> BRACKET[지문 구간 표시 삽입]
    BRACKET --> PTXT[지문 텍스트]
    ...
    %% 4 유사문제 추천 Hybrid
    QTXT --> REC[유사문제 추천 Hybrid]

    REC --> UTAG[사용자 태깅 JSON\nGPT4o mini]
    UTAG --> SCORE[점수 계산\ntag 07 embed 03]
    ...
    SCORE --> PATH[이미지 경로 매핑]
    IMGROOT --> PATH

    %% Output
    GEN --> OUT[응답 JSON\nquestion passage response similar problems]
    CON --> OUT
    PATH --> OUT
    PTXT --> OUT
    QTXT --> OUT
