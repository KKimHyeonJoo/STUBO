...
- 각 과목에 최적화된 파이프라인 모듈 실행  
  - 문학: GPT-4o 기반 이미지 OCR + LangChain RAG 기반 정답/해설 생성 + GPT 기반 태깅 → FAISS 유사 문제 추천
  - 비문학: EasyOCR 기반 지문/문제 인식 → GPT-4o 기반 문제 해결 및 해설 생성 → GPT 태깅 → Sentence-BERT 임베딩 기반 유사도 계산 및 추천 
  - 화법과 작문: EasyOCR로 지문/문제 추출 → GPT-4o 기반 정답 및 해설 생성 → FAISS 기반 임베딩 유사도 검색으로 유사 문제 추천
  - 언어와 매체: 
...
- 문학 : `<보기>`와 지문 분석을 통한 논리적 선택지 검토 및 오답 해설 제공
